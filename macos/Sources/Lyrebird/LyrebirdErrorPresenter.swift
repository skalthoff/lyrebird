...
/// Maps a `LyrebirdError` surfaced by the core (via UniFFI) into a
/// presentation-quality localized message.
///
/// Issue #351. The core declares `LyrebirdError` as a UniFFI `flat_error`, so on
...
/// both of which might be `LyrebirdError::Server`). When the mapped variant
/// already carries enough context (Auth, NotFound, RateLimit) the `context`
/// parameter is ignored.
enum LyrebirdErrorPresenter {
    ...
    static func key(for error: Error, context: Context = .generic) -> String {
        ...
        // `LyrebirdError::Server` still means "credentials are bad", and
